...
func getGameRules(_ context.Context, _ *mcp.CallToolRequest, _ GetGameRulesInput) (*mcp.CallToolResult, GetGameRulesOutput, error) {
	...
	out := GetGameRulesOutput{
		...
		DifficultyGuidance: "simulate_run (headless simulation with a scripted bot) is not implemented yet — it lands in a future phase. Until then, use validate_map for structural and connectivity checks, and use judgment on difficulty: shorter lanes (see the path_lengths validate_map returns) and lanes far from towers' effective coverage are harder; longer lanes with more room to build towers are easier.",
		Notes: []string{
